...
public class LeverControllerUptoBack : MonoBehaviour
{
    ...
    private Vector3 dorStartPosition; // ตำแหน่งเริ่มต้นของ Dor

    private float rotationAmount = -45f; // จำนวนองศาที่จะหมุนคันโยก
    private float currentRotation = 0f; // ค่าการหมุนปัจจุบันของคันโยก
    ...
    private bool isMovingToFinalPosition = true; // ตรวจสอบว่าประตูกำลังเลื่อนไปยังตำแหน่งสุดท้ายหรือไม่
    ...
    void Start()
    {
        ...
        dorStartPosition = Dor.transform.position; // บันทึกตำแหน่งเริ่มต้นของ Dor
    ...
    void Update()
    {
        ...
        {
            // สลับทิศทางการหมุน
            rotationAmount = isMovingToFinalPosition ? -45f : 45f;

            // หมุนคันโยก
            transform.Rotate(0, 0, rotationAmount);
            //audioManager.PlaySFX(audioManager.Lever); // เล่นเสียงโยกคันโยก

            // เริ่มเลื่อน Dor
            isLeverPulled = true; // คันโยกถูกโยกแล้ว
        }

        // ถ้าคันโยกถูกโยกแล้ว และประตูยังไม่ถึงตำแหน่งสุดท้าย ให้เลื่อน Dor
        if (isLeverPulled)
        {
    ...
    void MoveDor()
    {
        Vector3 targetPosition = isMovingToFinalPosition ? dorFinalPosition : dorStartPosition;

        // เลื่อน Dor ไปยังตำแหน่งปลายทางหรือตำแหน่งเริ่มต้น
        Dor.transform.position = Vector3.MoveTowards(Dor.transform.position, targetPosition, moveSpeed * Time.deltaTime);

        // ตรวจสอบว่า Dor ถึงตำแหน่งปลายทางแล้วหรือยัง
        if (Dor.transform.position == targetPosition)
        {
            isLeverPulled = false; // หยุดการเลื่อน Dor เมื่อถึงปลายทาง
            isMovingToFinalPosition = !isMovingToFinalPosition; // สลับสถานะการเลื่อน (ไปหรือกลับ)
            Debug.Log(isMovingToFinalPosition ? "Dor เลื่อนกลับไปที่ตำแหน่งเริ่มต้นแล้ว" : "Dor เลื่อนลงจนสุดแล้ว");
        }
    }
